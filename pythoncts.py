...
CTS_RETRIES = 5
CTS_RETRY_WAIT_SECONDS = 3
CTS_REQUEST_TIMEOUT_SECONDS = 30


def cts_urlopenWithRetry(
    url,
    retries=CTS_RETRIES,
    wait_seconds=CTS_RETRY_WAIT_SECONDS,
    timeout=CTS_REQUEST_TIMEOUT_SECONDS,
):
    last_error = None
...
def cts_nslist():
    global ctsurl
    res = ""
    data = cts_urlopenWithRetry("https://urncts.eu/namespaceresolver/index.php")
    for line in data:
...
def cts_nsurl(ns):
    global ctsurl
    try:
        data = cts_urlopenWithRetry("https://urncts.eu/namespaceresolver/" + ns)
        for line in data:
...
def cts_requestFromCts(requesturl):
    ...
    print(cts_lastURL)
    try:
        data = cts_urlopenWithRetry(cts_lastURL)
        for line in data:
            res += line.decode("utf-8")
    except Exception:
        print(
